...
interface IHeroProps {
  readonly t: TDictionary["hero"];
}

export function Hero({ t }: IHeroProps) {
      ...
      >
        ...
        <h1 className="font-heading text-sand">
          {t.lines.map((line) => (
            <span key={line.text} className="block overflow-hidden pb-1">
              <span
                className={`block tracking-tight leading-[0.98] text-[clamp(2.6rem,8.5vw,7rem)] ${line.italic ? "italic text-clay-light" : ""}`}
              >
                {line.text}
              </span>
            </span>
        ...
        <div className="mt-12 grid grid-cols-1 gap-4 border-t border-sand/20 pt-6 sm:grid-cols-3">
          {t.metrics.map((metric) => (
            <div key={metric.id} data-testid={`hero-metric-${metric.id}`}>
